...
public static class ReturningUpdateExpressionExtensions
{
    public static readonly MethodInfo InternalUpdatedMethodInfo
        = typeof(ReturningUpdateExpressionExtensions)
            ...
            .Single(m =>
                m.Name == nameof(InternalUpdated) &&
                m.GetParameters()[1].ParameterType == typeof(IReadOnlyList<ITuple>) &&
                m.GetParameters()[2].ParameterType.GetGenericArguments()[0].GetGenericTypeDefinition() == typeof(Func<,>));

    public static readonly MethodInfo InternalUpdated2MethodInfo
        = typeof(ReturningUpdateExpressionExtensions)
            .GetMethods(BindingFlags.NonPublic | BindingFlags.Static)
            .Single(m =>
                m.Name == nameof(InternalUpdated) &&
    ...
    private static IQueryable<TResult> InternalUpdated<TSource, TResult>(
        this IQueryable<TSource> source,
    ...
    {
        var methodCallExpression = Expression.Call(
            method: InternalUpdated2MethodInfo.MakeGenericMethod(typeof(TSource), typeof(TResult)),
            arg0: source.Expression,
            arg1: setters,
            arg2: Expression.Quote(selector),
            arg3: stubSelectSource.Expression,
